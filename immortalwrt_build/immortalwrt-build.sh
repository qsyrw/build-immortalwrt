...
cleanup_menu() {
    while true; do
        clear
        echo "🗑️ 清理工具"
        echo "1) 按配置清理  2) 按日期清理  3) 清空所有  R) 返回"
        read -p "选择: " choice
        case $choice in
            1) 
                local confs=($(find "$CONFIGS_DIR" -name "*.conf" -exec basename {} .conf \;))
                for i in "${!confs[@]}"; do echo "$((i+1))) ${confs[$i]}"; done
                read -p "序号: " idx
                local target="${confs[$((idx-1))]}"
                [[ -n "$target" ]] && find "$OUTPUT_DIR" -name "*_${target}_*.zip" -delete && echo "已清理"
                read -p "..." ;;
            2) 
                read -p "天数: " days
                [[ "$days" =~ ^[0-9]+$ ]] && find "$OUTPUT_DIR" -name "*.zip" -mtime +"$days" -delete && echo "已清理"
                read -p "..." ;;
            3) 
                read -p "确认清空 (YES)? " c
                [[ "$c" == "YES" ]] && rm -f "$OUTPUT_DIR"/*.zip && echo "已清空"
                read -p "..." ;;
            R|r) return ;;
        esac
    done
...
# ... (archive_firmware_and_logs 和其他辅助函数已经在上面定义或简化)
